...
class PastebinConverter(StringToEmbed):
    async def convert(self, ctx: commands.Context, argument: str) -> str:
        ...
        async with ctx.cog.session.get(f"https://pastebin.com/raw/{paste_id}") as resp:
            if resp.status != 200:
                raise commands.BadArgument(f"`{argument}` is not a valid Pastebin link.")
            embeddata = await resp.text()
        return await super().convert(ctx, embeddata)

class PastebinConverterWebhook(ListStringToEmbed):
    async def convert(self, ctx: commands.Context, argument: str) -> str:
        match = PASTEBIN_RE.match(argument)
        if not match:
            raise commands.BadArgument(f"`{argument}` is not a valid Pastebin link.")
